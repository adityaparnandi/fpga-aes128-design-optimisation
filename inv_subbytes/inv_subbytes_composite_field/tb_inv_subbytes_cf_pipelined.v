...
// -----------------------------------------------------------------------------
// Testbench for AES-128 inverse SubBytes using the composite-field implementation
//
// Verification method:
//   - Drives all 256 possible 8-bit input values into inv_subbytes_cf_pipelined.
//   - Uses the known-correct AES inverse S-box LUT as the golden reference.
//   - Accounts for the DUT pipeline latency by delaying the expected outputs through an expected-data queue and valid queue.
//   - Compares each DUT output byte against the LUT reference once valid.
// -----------------------------------------------------------------------------
module tb_inv_subbytes_cf_pipelined;

// Core pipeline parameters for inv_subbytes_cf_pipelined
localparam integer REG_IN         = 1;
localparam integer REG_INV_AFF    = 1;
...
localparam integer LAT =
    REG_IN + REG_INV_AFF + REG_ISO + REG_SPLIT + REG_DEN +
    REG_DEN2 + REG_DENINV_SQ + REG_DENINV_M1 + REG_DENINV_PP +
    REG_DENINV + REG_MUL_PP + REG_MUL + REG_OUT;
...
// AES inverse S-box reference table
...
// AES inverse S-box LUT used as reference
initial begin
...
// Stimulus, queue initialisation, and PASS/FAIL reporting to Tcl console
initial begin
    errors   = 0;
    checked  = 0;
    in_byte  = 8'h00;
    ...
    $display("Starting CF InvSubBytes verification...");
    ...
    if (errors == 0 && checked == TOTAL_TESTS)
        $display("PASS: CF InvSubBytes matches AES inverse S-box for all %0d inputs (LAT=%0d).", TOTAL_TESTS, LAT);
    else
...
always @(negedge clk) begin
    if (valid_q[LAT-1]) begin
        ...
        if (out_byte !== exp_q[LAT-1]) begin
            errors = errors + 1;
            $display("Mismatch: expected=%02h got=%02h (check=%0d)",
                     exp_q[LAT-1], out_byte, checked);
        end
    end
end
