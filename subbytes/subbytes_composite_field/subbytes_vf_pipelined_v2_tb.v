...
// -----------------------------------------------------------------------------
// Testbench for AES-128 forward SubBytes using the composite-field implementation
//
// Verification method:
//   - Drives all 256 possible 8-bit input values into subbytes_cf_pipelined.
//   - Uses the known-correct AES forward S-box LUT as the golden reference.
//   - Accounts for the DUT pipeline latency by delaying the expected outputs
//     through an expected-data queue and valid queue.
//   - Compares each DUT output byte against the LUT reference once valid.
//
// Brief working:
//   1. Configure the DUT with the same pipeline register settings used in the
//      implementation under test.
//   2. Compute the total DUT latency LAT from the enabled register stages.
//   3. Apply input bytes 0x00 to 0xFF in sequence.
//   4. Push the expected LUT outputs through a matching latency queue.
//   5. Report mismatches and print a final PASS/FAIL summary.
//
// This testbench verifies functional correctness only.
// It does not measure timing/Fmax directly.
// -----------------------------------------------------------------------------
module tb_subbytes_cf_verify;

// Core pipeline parameters for subbytes_cf_pipelined (register activations)
localparam integer REG_IN         = 1;
localparam integer REG_ISO        = 1;
localparam integer REG_SPLIT      = 1;
localparam integer REG_DEN        = 1;
localparam integer REG_DEN2       = 1;
localparam integer REG_DENINV_SQ  = 1;
localparam integer REG_DENINV_M1  = 1;
localparam integer REG_DENINV_PP  = 1;
localparam integer REG_DENINV     = 1;
localparam integer REG_DENINV_DUP = 1;
localparam integer REG_MUL_PP     = 1;
localparam integer REG_MUL        = 1;
localparam integer REG_AFF        = 1;
localparam integer REG_OUT        = 1;

// Total DUT latency in clock cycles.
// This equals the number of enabled pipeline register stages that lie on the
// input-to-output datapath. The expected LUT outputs are delayed by the same
// number of cycles so that they line up correctly with the DUT output.
localparam integer LAT =
    REG_IN + REG_ISO + REG_SPLIT + REG_DEN + REG_DEN2 +
    REG_DENINV_SQ + REG_DENINV_M1 + REG_DENINV_PP + REG_DENINV +
    REG_MUL_PP + REG_MUL + REG_AFF + REG_OUT;

// Number of test inputs and extra cycles used to flush the pipeline
localparam integer TOTAL_TESTS  = 256;
localparam integer FLUSH_CYCLES = LAT + 4;
...
// Golden AES forward S-box reference table
reg [7:0] sbox [0:255];

// Expected-data queue and valid queue for latency alignment.
// exp_q stores the expected LUT outputs as they move through a model of the
// DUT pipeline, and valid_q indicates when comparison is safe.
reg [7:0] exp_q [0:LAT-1];
reg       valid_q [0:LAT-1];
...
// AES forward S-box LUT used as golden reference
initial begin
...
// Stimulus, queue initialisation, pipeline flush and final PASS/FAIL reporting
initial begin
    errors   = 0;
    checked  = 0;
    cycle    = 0;
    ...
    $display("Starting CF SubBytes verification...");
    $display("Configured latency LAT = %0d cycles", LAT);
    ...
    if (checked != TOTAL_TESTS)
        $display("WARNING: checked=%0d, expected=%0d", checked, TOTAL_TESTS);

    if (errors == 0 && checked == TOTAL_TESTS)
        $display("PASS: CF SubBytes matches AES S-box for all %0d inputs (LAT=%0d).", TOTAL_TESTS, LAT);
    else
        $display("FAIL: %0d mismatches, checked=%0d, expected=%0d, LAT=%0d.",
                 errors, checked, TOTAL_TESTS, LAT);

    $finish;
end

// Shift expected-data queue and valid queue each cycle
always @(posedge clk) begin
    cycle <= cycle + 1;

    exp_q[0]   <= sbox[in_byte];
    valid_q[0] <= valid_in;

    for (k = LAT-1; k >= 1; k = k - 1) begin
        exp_q[k]   <= exp_q[k-1];
        valid_q[k] <= valid_q[k-1];
    end
end

// Compare DUT output against the latency-adjusted LUT reference
always @(negedge clk) begin
    if (valid_q[LAT-1]) begin
        checked = checked + 1;
        if (out_byte !== exp_q[LAT-1]) begin
            errors = errors + 1;
            $display("Mismatch: expected=%02h got=%02h (check=%0d, cycle=%0d)",
                     exp_q[LAT-1], out_byte, checked, cycle);
        end
    end
end
